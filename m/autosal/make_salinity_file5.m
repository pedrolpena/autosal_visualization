function output = make_salinity_file5 (salts)
%
...
%   added.
% Version 5: Added cast number column (9) if it is present in the
% structure.
%
...
if ~isempty(salts(1).txt_salinity_correction)
    ...
    output_salinity_correction = salts(1).txt_salinity_correction;
    for II = 2: length(salts);
        output_salinity_correction = [output_salinity_correction' salts(II).txt_salinity_correction']';
    end;

    if ~isempty(salts(1).txt_castnumber)
        clear output_castnumber
        output_castnumber = salts(1).txt_castnumber;
        for II = 2: length(salts);
            output_castnumber = [output_castnumber' salts(II).txt_castnumber']';
        end;
        output = [output_sta output_niskin output_bottle ...
            output_twiceratio output_salinity output_quality output_castnumber output_salinity_correction output_castnumber];
    else
        output = [output_sta output_niskin output_bottle ...
            output_twiceratio output_salinity output_quality output_castnumber output_salinity_correction];
    end
else
    if ~isempty(salts(1).txt_castnumber)
        clear output_castnumber
        ...
        output = [output_sta output_niskin output_bottle ...
            output_twiceratio output_salinity output_quality output_castnumber output_castnumber];
    else
        output = [output_sta output_niskin output_bottle ...
            output_twiceratio output_salinity output_quality output_castnumber];
    end
end
...
%
% Station -9 is a flag value - missing data
%
...
%
% Station 888 is substandard read value
%
II = find(output_sta == 88);
if ~isempty(II)
    output_sta (II,:) = [];
    output (II,:) = [];
end
